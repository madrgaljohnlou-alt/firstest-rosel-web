...
const ManageProductsPage = () => {
	...
	const [activeTab, setActiveTab] = useState("create"); // create | update | monitor
		...
		<AdminLayout>
			<div className='py-8 px-4'>
				...
				<div className='mb-6 flex items-center gap-2'>
					<button onClick={()=>setActiveTab('create')} className={`px-3 py-2 rounded ${activeTab==='create' ? 'bg-[#901414] text-[#feffff]' : 'bg-[#82695b] text-[#feffff]'}`}><PlusCircle className='inline mr-2' />Create</button>
					<button onClick={()=>setActiveTab('update')} className={`px-3 py-2 rounded ${activeTab==='update' ? 'bg-[#901414] text-[#feffff]' : 'bg-[#82695b] text-[#feffff]'}`}><Edit3 className='inline mr-2' />Update</button>
					<button onClick={()=>setActiveTab('monitor')} className={`px-3 py-2 rounded ${activeTab==='monitor' ? 'bg-[#901414] text-[#feffff]' : 'bg-[#82695b] text-[#feffff]'}`}><Eye className='inline mr-2' />Monitor</button>
					<button onClick={()=>setActiveTab('activity')} className={`px-3 py-2 rounded ${activeTab==='activity' ? 'bg-[#901414] text-[#feffff]' : 'bg-[#82695b] text-[#feffff]'}`}><History className='inline mr-2' />Activity Log</button>
				</div>
					...
					<motion.div className='bg-[#feffff] shadow-lg rounded-lg p-6 border border-[#82695b]' initial={{opacity:0,y:20}} animate={{opacity:1,y:0}}>
						<form onSubmit={onCreate} className='grid grid-cols-1 md:grid-cols-2 gap-4'>
							<div>
								...
								<input value={newProduct.name} onChange={(e)=>setNewProduct({...newProduct, name: e.target.value})} className='w-full bg-[#f8f3ed] border border-[#82695b] rounded px-3 py-2 text-[#82695b] focus:ring-2 focus:ring-[#901414] focus:border-transparent' required />
							</div>
							<div>
								<label className='block text-sm text-[#82695b] mb-1 font-medium'>Category</label>
								<select value={newProduct.category} onChange={(e)=>setNewProduct({...newProduct, category: e.target.value})} className='w-full bg-[#f8f3ed] border border-[#82695b] rounded px-3 py-2 text-[#82695b] focus:ring-2 focus:ring-[#901414] focus:border-transparent' required>
							...
							<div>
								...
								<input type='number' step='0.01' value={newProduct.price} onChange={(e)=>setNewProduct({...newProduct, price: e.target.value})} className='w-full bg-[#f8f3ed] border border-[#82695b] rounded px-3 py-2 text-[#82695b] focus:ring-2 focus:ring-[#901414] focus:border-transparent' required />
							</div>
							<div>
								<label className='block text-sm text-[#82695b] mb-1 font-medium'>Stocks (quantity)</label>
								<input type='number' min='0' value={newProduct.quantity} onChange={(e)=>setNewProduct({...newProduct, quantity: e.target.value})} className='w-full bg-[#f8f3ed] border border-[#82695b] rounded px-3 py-2 text-[#82695b] focus:ring-2 focus:ring-[#901414] focus:border-transparent' required />
							</div>
							<div>
								<label className='block text-sm text-[#82695b] mb-1 font-medium'>Barcode</label>
								<div className='flex items-center gap-2 mb-2'>
									<select value={createBarcodeMode} onChange={(e)=>setCreateBarcodeMode(e.target.value)} className='bg-[#f8f3ed] border border-[#82695b] rounded px-2 py-2 text-[#82695b] focus:ring-2 focus:ring-[#901414] focus:border-transparent'>
									...
									{createBarcodeMode !== 'camera' && (
										<input value={newProduct.barcode} onChange={(e)=>setNewProduct({...newProduct, barcode: e.target.value})} placeholder='Scan or enter barcode' className='w-full bg-[#f8f3ed] border border-[#82695b] rounded px-3 py-2 text-[#82695b] focus:ring-2 focus:ring-[#901414] focus:border-transparent' />
									)}
									{createBarcodeMode === 'camera' && (
										<div className='bg-[#f8f3ed] border border-[#82695b] rounded p-3'>
											<div className='flex items-center gap-2 mb-2'>
												...
												<button type='button' onClick={()=>{ try{ window.__createScanControls?.stop?.(); }catch{} }} className='px-3 py-2 bg-[#82695b] border border-[#82695b] rounded text-[#feffff] hover:bg-[#6b5649]'>Stop</button>
											</div>
											<video id='create-scan-video' style={{ width: 240, height: 160 }} muted playsInline />
											<div className='text-xs text-[#82695b] mt-2'>Last scanned: {lastScannedCreate || '—'}</div>
										</div>
									)}
							</div>
							<div className='md:col-span-2'>
								<label className='block text-sm text-[#82695b] mb-1 font-medium'>Description</label>
								<textarea rows='3' value={newProduct.description} onChange={(e)=>setNewProduct({...newProduct, description: e.target.value})} className='w-full bg-[#f8f3ed] border border-[#82695b] rounded px-3 py-2 text-[#82695b] focus:ring-2 focus:ring-[#901414] focus:border-transparent' required />
							</div>
							<div className='md:col-span-2'>
								<label className='block text-sm text-[#82695b] mb-2 font-medium'>Images</label>
								<input type='file' accept='image/*' multiple onChange={handleCreateImageChange} className='hidden' id='create-images' />
								<label htmlFor='create-images' className='inline-flex items-center gap-2 px-3 py-2 bg-[#82695b] border border-[#82695b] rounded text-[#feffff] cursor-pointer hover:bg-[#6b5649] transition-colors'><Upload className='h-4 w-4' /> Upload Images</label>
								<div className='mt-2 flex flex-wrap gap-2'>
									{newProduct.images.map((img, idx)=> (
										<img key={idx} src={img} alt='preview' className='w-16 h-16 object-cover rounded border border-[#82695b]' />
									))}
								</div>
							</div>
							<div className='md:col-span-2'>
								<button type='submit' disabled={loading} className='w-full bg-[#901414] hover:bg-[#7a0f0f] text-[#feffff] font-semibold py-2 rounded flex items-center justify-center transition-colors disabled:opacity-50'>
					...
					<motion.div className='bg-[#feffff] shadow-lg rounded-lg p-6 border border-[#82695b]' initial={{opacity:0,y:20}} animate={{opacity:1,y:0}}>
						<div className='grid grid-cols-1 lg:grid-cols-3 gap-6'>
							...
							<div className='lg:col-span-1'>
								<h3 className='text-lg font-semibold text-[#901414] mb-3'>Select Product</h3>
								...
								<input
									...
									<div className='grid grid-cols-2 gap-2'>
										<select 
											...
											className='bg-[#f8f3ed] border border-[#82695b] rounded px-2 py-2 text-[#82695b] focus:ring-2 focus:ring-[#901414] focus:border-transparent text-sm'
										>
								...
								<div className='flex items-center gap-2 mb-2'>
									...
									<select value={updateSearchMode} onChange={(e)=>setUpdateSearchMode(e.target.value)} className='bg-[#f8f3ed] border border-[#82695b] rounded px-2 py-1 text-[#82695b] focus:ring-2 focus:ring-[#901414] focus:border-transparent'>
										<option value='manual'>Manual</option>
										<option value='usb'>USB Scanner</option>
										<option value='camera'>Camera</option>
									</select>
								...
								{updateSearchMode === 'camera' && (
									<div className='bg-[#f8f3ed] border border-[#82695b] rounded p-3 mb-2'>
										<div className='flex items-center gap-2 mb-2'>
											...
											<button type='button' onClick={()=>{ try{ window.__searchScanControls?.stop?.(); }catch{} }} className='px-3 py-2 bg-[#82695b] border border-[#82695b] rounded text-[#feffff] hover:bg-[#6b5649]'>Stop</button>
										</div>
										<video id='search-scan-video' style={{ width: 240, height: 160 }} muted playsInline />
										<div className='text-xs text-[#82695b] mt-2'>Last scanned: {lastScannedSearch || '—'}</div>
									</div>
								)}
								{/* Sort Controls */}
								<div className='mb-2 flex items-center justify-between'>
									<span className='text-xs text-[#82695b]'>Sort by:</span>
									<div className='flex gap-1'>
										<button 
											onClick={() => handleUpdateSort('name', updateSortKey === 'nameAsc' ? 'desc' : 'asc')}
											className={`px-2 py-1 text-xs rounded transition-colors ${
												updateSortKey === 'nameAsc' || updateSortKey === 'nameDesc' 
													? 'bg-[#901414] text-[#feffff]' 
													: 'bg-[#f8f3ed] text-[#82695b] hover:bg-[#901414] hover:text-[#feffff]'
											}`}
										>
								...
								<div className='max-h-[420px] overflow-y-auto space-y-2'>
									{filteredUpdateProducts.map((p)=> (
										<button key={p._id} className={`w-full text-left p-3 rounded border ${selectedProductId===p._id ? 'border-[#901414] bg-[#f8f3ed]' : 'border-[#82695b] bg-[#feffff] hover:bg-[#f8f3ed]'}`} onClick={()=>setSelectedProductId(p._id)}>
											<div className='flex items-center gap-3'>
												<img src={p.image} alt={p.name} className='w-12 h-12 object-cover rounded' />
												<div className='flex-1'>
													<div className='text-[#82695b] text-sm font-medium'>{p.name}</div>
													<div className='text-xs text-[#82695b]'>{p.category} • ₱{p.price} • {p.quantity} in stock</div>
												</div>
											</div>
										</button>
									))}
