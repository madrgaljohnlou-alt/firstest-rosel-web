	...
	return (
		isAdmin ? (
			...
			<header className={`fixed top-0 left-0 w-full bg-transparent backdrop-blur-3xl shadow-lg z-50 transition-all duration-300 border-white ${isNavbarVisible ? 'translate-y-0' : '-translate-y-full'}`}>
				...
				<aside
					className={`fixed top-0 left-0 h-screen bg-[#fbf4df] z-50 transition-all duration-300 ${
				...
				>
					<div className="flex items-center justify-between px-4 py-4 border-b-2 border-[#a31f17] bg-[#fbf4df]">
						{!isSidebarCollapsed && (
							<span className="text-lg font-bold text-[#860809]">Admin Menu</span>
						)}
					...
					<nav className="flex-1 flex flex-col px-2 py-4 gap-1 bg-[#fbf4df]">
						<Link
							to="/dashboard"
							className={`flex items-center rounded-md font-medium transition-colors duration-200 bg-transparent text-[#030105] hover:bg-[#a31f17] hover:text-[#fffefc] ${
						...
						>
							<ClipboardList className={`h-5 w-5 ${!isSidebarCollapsed ? 'mr-3' : ''}`} />
							{!isSidebarCollapsed && "Order Management"}
						</Link>
						<Link
							to="/pos"
							className={`flex items-center rounded-md font-medium transition-colors duration-200 bg-transparent text-[#030105] hover:bg-[#a31f17] hover:text-[#fffefc] ${
						...
						>
							<RefreshCw className={`h-5 w-5 ${!isSidebarCollapsed ? 'mr-3' : ''}`} />
							{!isSidebarCollapsed && "Replacement Requests"}
						</Link>
						<Link
						...
						>
							<MessageCircle className={`h-5 w-5 ${!isSidebarCollapsed ? 'mr-3' : ''}`} />
							{!isSidebarCollapsed && "Chat Management"}
						</Link>
						<Link
							to="/coupons"
							className={`flex items-center rounded-md font-medium transition-colors duration-200 bg-transparent text-[#030105] hover:bg-[#a31f17] hover:text-[#fffefc] ${
							...
							title={isSidebarCollapsed ? "Coupons" : ""}
						>
							<Ticket className={`h-5 w-5 ${!isSidebarCollapsed ? 'mr-3' : ''}`} />
							{!isSidebarCollapsed && "Coupons"}
						</Link>
					</nav>
				</aside>
				...
				{/* Topbar for admin */}
				<header
					className={`fixed top-0 left-0 w-full h-16 bg-[#fffefc] border-b border-[#f7e9b8] z-40 flex items-center transition-all duration-300 ${isNavbarVisible ? 'translate-y-0' : '-translate-y-full'}`}
					style={{ boxShadow: '0 2px 16px rgba(3,1,5,0.04)', marginLeft: '0' }}
				>
					<div className={`flex items-center justify-between w-full px-4 transition-all duration-300 ${
						...
						<button
							...
							className="lg:hidden p-2 rounded hover:bg-[#f7e9b8] transition"
						...
						>
							...
							<Link
								to="/dashboard"
								className="text-2xl font-bold text-[#a31f17] flex items-center space-x-2"
						...
						<div className="flex items-center gap-4">
							...
							<div className="relative" ref={profileMenuRef}>
								<button
									...
									className="flex items-center justify-center w-10 h-10 rounded-full bg-[#f7e9b8] hover:bg-[#fffefc] border border-[#f7e9b8] overflow-hidden"
									aria-haspopup="menu"
								...
								>
									{(user?.profileImageUrl || user?.avatarUrl || user?.profileImage || user?.photoURL || user?.photo) ? (
										...
										<User className="text-[#a31f17]" size={20} />
									)}
								</button>
								...
								{isProfileMenuOpen && (
									<div className="absolute right-0 mt-2 w-64 bg-white rounded-md shadow-lg ring-1 ring-black ring-opacity-5 z-50">
										...
										<div className="py-1">
											...
											<Link to="/notifications" className="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100" onClick={() => setIsProfileMenuOpen(false)}>Notifications</Link>
											...
											<button className="w-full text-left block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100" onClick={() => setIsProfileMenuOpen(false)}>Activity Log</button>
											<div className="my-1 border-t border-gray-200" />
											<button className="w-full text-left block px-4 py-2 text-sm text-[#a31f17] hover:bg-gray-100 flex items-center" onClick={() => { setIsProfileMenuOpen(false); logout(); }}>
				...
				<>
					...
					<div className={`fixed inset-0 z-50 lg:hidden ${isMobileMenuOpen ? 'block' : 'hidden'}`}>
						...
						<div className={`fixed top-0 left-0 h-full w-64 bg-[#f8f3ed] shadow-lg transform transition-transform duration-300 ${
							...
							<div className="flex items-center justify-between px-4 py-4 border-b border-[#f7e9b8]">
								<span className="text-lg font-bold text-[#901414]">Menu</span>
								<button
									onClick={() => setIsMobileMenuOpen(false)}
									className="p-2 rounded hover:bg-[#f7e9b8] transition"
									aria-label="Close menu"
								>
									<X className="text-[#901414]" />
								</button>
							</div>
					...
					<header className={`fixed top-0 left-0 w-full bg-[#f8f3ed] z-50 transition-all duration-300 ${isNavbarVisible ? 'translate-y-0' : '-translate-y-full'}`}>
						<div className='container mx-auto px-4 sm:px-8 lg:px-12 py-3 sm:py-4 lg:py-6'>
							<div className='flex flex-wrap justify-between items-center '>
								...
								<div className="flex items-center space-x-1 sm:space-x-2 lg:space-x-3">
									...
									<Link to='/welcome' className='text-lg sm:text-xl lg:text-2xl font-bold text-[#901414] items-center space-x-2 flex'>
										Rosel Frozen Meats
									</Link>
								</div>
								...
								<nav className='hidden lg:flex flex-wrap items-center [&>*]:mx-5 gap-4 justify-center flex-grow '>
									<Link
										to={"/welcome"}
										className='text-[#901414] font-semibold hover:text-[#810e0e] transition duration-300 ease-in-out hover:underline hover:underline-offset-8 hover:decoration-[#810e0e]'
									>
										Home
									</Link>
