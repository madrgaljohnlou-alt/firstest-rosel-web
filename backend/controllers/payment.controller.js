...
export const createCheckoutSession = async (req, res) => {
    try {
        ...
        const { products, couponCode, shippingInfo, shippingMethod, lalamoveQuote, finalTotal } = req.body;
        ...
        // Optional coupon validation and Stripe coupon creation
        ...
        if (couponCode) {
            ...
            stripeCouponId = await createStripeCoupon(validation.discountType, validation.discountAmountCentsOrPercent);
...
async function createStripeCoupon(type, amountOrPercent) {
    // type: "percent" | "fixed"
    if (type === "percent") {
        const coupon = await stripe.coupons.create({
            duration: "once",
            percent_off: amountOrPercent,
        });
        return coupon.id;
    }
    const coupon = await stripe.coupons.create({
        duration: "once",
        amount_off: amountOrPercent, // in cents
        currency: "php",
    });
    return coupon.id;
}
